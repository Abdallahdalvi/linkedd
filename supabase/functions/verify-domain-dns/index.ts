...
serve(async (req) => {
  ...
  try {
    ...
    try {
      const aRecordResponse = await fetch(`https://cloudflare-dns.com/dns-query?name=${domain.domain}&type=A`, {
      ...
      try {
        ...
        if (txtRecordData.Answer && txtRecordData.Answer.length > 0) {
          ...
          if (hasValidToken) {
            ...
            errors.push(
              `TXT record at ${TXT_RECORD_NAME}.${domain.domain} does not contain correct verification token`,
            );
    ...
    if (updateError) {
      ...
      return new Response(JSON.stringify({ success: false, error: "Failed to update domain status" }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    return new Response(
      JSON.stringify({
        ...
        nextStep: dnsVerified ? "Your domain is ready to use." : "Please check your DNS settings and try again.",
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (error: unknown) {
